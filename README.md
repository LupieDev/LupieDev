<div align="center">

# Hi, I'm Lukas 👋

### Fachinformatiker Daten-/Prozessanalyse | IT Specialist Data & Process Analytics

**Data Analytics · Data Engineering · Business Intelligence · Cloud Engineering · Game Development**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/lukaspietsch/)
[![Email](https://img.shields.io/badge/Email-Contact-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:pietsch.lukas@gmail.com)

</div>

---

## 👨‍💻 About Me

I am an IHK certified IT specialist for data and process analytics (_Fachinformatiker Daten-/Prozessanalyse_) specializing in **Data Analytics & Engineering**.

My strong analytics foundation allows me to understand how data and databases are structured. I apply this knowledge to design efficient database systems and construct automated pipelines directly into the analytics cycle.

I am a self-driven, adaptable learner who loves building databases, juggling numbers, and mastering cloud infrastructure.

On the side, I am learning game development and analytics basics using the Godot Engine.

---

## 🎯 Analytics & Engineering Roadmap (2026/2027)

_I am actively executing a structured career roadmap to bridge the gap between apprenticeship and job entry:_

| Phase | Title                       | Focus & Core Technologies                                      |     Status     |
| :---: | :-------------------------- | :------------------------------------------------------------- | :------------: |
| **1** | **Basics & Analytics**       | Python, SQL, Statistics, Data Modelling                  | 🔄 In Progress |
| **2** | **Data Warehouse**        |   Warehousing Architectures, Snowlake, PostgreSQL, ETL Process            | 🔄 In Progress |
| **3** | **Big Data**   | Cluster Management, Apache, Kubernetes, Docker        |   ⏳ Planned   |
| **4** | **Analytics Deep Dive**       | Descriptive Analysis, Statistical Analysis |   ⏳ Planned   |
| **5** | **Cloud Engineering**        | Data Storage Solutions, Azure, AWS, Google Cloud     |   ⏳ Planned   |
| **6** | **Machine Learning** | Neural Networks, Algorithms, Deep Learning Frameworks     |   ⏳ Planned   |

---

## 📂 Highlight Projects

### 🤖 Apprenticeship Final Project: GitHub Data Pipeline

_My capstone project: A pipeline using the GitHub API to extract, clean and visualise repository activity data_

- **Stack:** Python · GitHub API · Pandas · Matplotlib · CSV
- **Repositories:** (https://github.com/LupieDev/GitHub_Activity) 

### 📊 Analysing data from the Titanic

_Analysing and visualising survivor data from the Titanic_

- **Stack:** Python · Pandas · Seaborn · Matplotlib 
- **Repositories:** (https://github.com/LupieDev/school/blob/main/scr/Titanic_Tagesaufgabe.ipynb) 

### 📚 Palmer Penguin Species Analysis

_Analysing data from the palmer penguin species and training basic machine learning models for prediction_

- **Stack:** Python · Pandas · Seaborn · Sklearn
- **Repositories:** (https://github.com/LupieDev/school/blob/main/scr/penguins_pietsch.ipynb) 

### 📚 Recipe Website and Database

_A basic website connected to a database storing recipes with the ability to like and comment_

- **Stack:** Javascript · SQL · HTML
- **Repositories:** (https://github.com/LupieDev/javascript) 

### 📚 Game Development in Godot Engine

_Using a tutorial I created a Frogger-like and a minigame on itch_

- **Stack:** Godot Engine · GDScript
- **Repositories:** (https://github.com/LupieDev/ballpoint_destroyer)
- **Itch Release:** (https://lupiedev.itch.io/ballpoint-destroyer) 

### 📊 Data Warehouse Project

_Conception of a Data Warehouse and pipeline_

- **Stack:** Python · PostgreSQL · PowerBI
- **Status:** `[ Status: In Progress ]`

### 📊 DDoSteroid

_Creating my next game using the Godot Engine, a multidirectional shooter inspired by the classic game Asteroids_

- **Stack:** Godot Engine · GDScript
- **Status:** `[ Status: In Progress ]`

---

## 🛠️ Tech Stack & Skills

### Currently Using & Solid:

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/-SQL-000?&logo=MySQL&logoColor=4479A1)
![Matplotlib](https://img.shields.io/badge/-Matplotlib-000000?style=flat&logo=python)
![Pandas](https://img.shields.io/badge/-Pandas-333333?style=flat&logo=pandas)
![Numpy](https://img.shields.io/badge/-Numpy-013243?&logo=NumPy)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikit-learn&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)


### Active Learning Focus:

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)


### Planned Roadmap Skills:

![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![Microsoft Azure](https://img.shields.io/badge/Microsoft_Azure-0089D6?style=flat-square&logo=microsoft-azure&logoColor=white)


---

## 🎯 Target Roles

I am aiming to step into roles that are about handling big data, by either creating pipelines and warehouses or analysing numbers. My interests are varied: Fintech, Life Sciences, (e)Sports and Video Game Development

- **Data/Business Intelligence Analyst**
- **Data Engineer**
- **Cloud Engineer**
- **Game Analyst**

---

## 📫 Let's Connect

[![Email](https://img.shields.io/badge/Email-pietsch.lukas@gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:pietsch.lukas@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/lukaspietsch/)


<!--
**lupie2/lupie2** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
